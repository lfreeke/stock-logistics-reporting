...
class Product(models.Model):
    ...
    def _compute_quantities_in_dict(self, from_date):
        ...
        domain_base = [("product_id", "in", self.ids), ("date", ">=", from_date)]
        domain_in = (
            domain_locs[1]
            + domain_base
            + [("picking_id.picking_type_id.code", "in", ["incoming", "mrp_operation"])]
        )
        domain_return = (
            domain_locs[2]
            + domain_base
            + [("location_dest_id.usage", "in", ["supplier", "inventory"])]  # loss
        )
        Move = self.env["stock.move"]
        moves_in = {
            item["product_id"][0]: item["product_qty"]
            for item in Move.read_group(
                domain_in, ["product_id", "product_qty"], ["product_id"]
            )
        }
        moves_return = {
            item["product_id"][0]: item["product_qty"]
            for item in Move.read_group(
                domain_return, ["product_id", "product_qty"], ["product_id"]
            )
        }
        return {x.id: (moves_in.get(x.id, 0) - moves_return.get(x.id, 0)) for x in self}
    ...
    def _compute_inventory_turn_report(self):
        ...
        available6m = {
            x.id: x.qty_available for x in self.with_context(to_date=at_6m_ago)
        }
        available12m = {
            x.id: x.qty_available for x in self.with_context(to_date=at_12m_ago)
        }
        ...
        for prod in self:
            ...
            prod.qty_consumed_6m = (
                prod.qty_available_6m + prod.qty_gotten_6m - prod.qty_available
            )
            prod.qty_consumed_12m = (
                prod.qty_available_12m + prod.qty_gotten_12m - prod.qty_available
            )
            # Months of Inventory
            prod.months_of_inventory_6m = (
                0.0
                if not prod.qty_consumed_6m
                else prod.qty_available / prod.qty_consumed_6m * 6
            )
            prod.months_of_inventory_12m = (
                0.0
                if not prod.qty_consumed_12m
                else prod.qty_available / prod.qty_consumed_12m * 12
            )
            # Turns / Cycles per Month
            prod.inventory_turns_6m = (
                0.0
                if not prod.months_of_inventory_6m
                else 12 / prod.months_of_inventory_6m
            )
            prod.inventory_turns_12m = (
                0.0
                if not prod.months_of_inventory_12m
